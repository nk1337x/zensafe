...
class SH_Train_Origin_Dataset(Dataset):
    ...
    def sample_feat(self, feat, labs, vid_type="Normal"):
        ...
        if self.sample == "uniform":
            if (feat_len - self.part_len) // (self.part_num + 1) < 1:
                ...
                move = np.random.randint(
                    (feat_len - self.part_len) // (self.part_num + 1)
                )
            # we want to have
            chosen = (
                np.linspace(
                    0, feat_len - self.part_len, num=self.part_num + 1, dtype=int
                )
                + move
            )
            chosen = chosen.repeat(self.part_len).reshape(
                [-1, self.part_len]
            ) + np.arange(0, self.part_len, 1, dtype=int)
        else:
            chosen = np.linspace(
                0, feat_len - self.part_len, num=self.part_num + 1, dtype=int
            )
            ...
            if chosen[1, 0] - chosen[0, 0] == 0:
                ...
                move = (
                    np.random.randint(
                        0, chosen[1, 0] - chosen[0, 0], [self.part_num + 1]
                    )
                    .repeat(self.part_len)
                    .reshape([-1, self.part_len])
                )
            chosen = chosen + move
        ...
        return (
            feat[chosen[: self.part_num * self.part_len], :],
            labs[chosen[: self.part_num * self.part_len]],
        )
    ...
    def __getitem__(self, item):
        ...
        norm_feat, norm_labs = self.sample_feat(
            self.norm_feats[norm_iter], norm_labs, vid_type="Normal"
        )
        abnorm_feat, abnorm_labs = self.sample_feat(
            self.abnorm_feats[abnorm_iter], abnorm_labs, vid_type="Abnormal"
        )

        return (
            torch.from_numpy(norm_feat).float(),
            torch.from_numpy(norm_labs).float(),
            torch.from_numpy(abnorm_feat).float(),
            torch.from_numpy(abnorm_labs).float(),
        )
