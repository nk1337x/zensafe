...
def cal_bcla(oversampledCrop):
    bcla = 0
    with open("configurations\sampling_configuration.pickle", "rb") as file:
    ...
    for key in labels_dict.keys():
        ...
        if key == "Normal":
            ...
            false_alarm = cal_false_alarm(
                np.array(score, dtype=float), np.array(labels_dict[key], dtype=float)
            )
            normal_far = false_alarm
            if logger == None:
                print(
                    "{}: \tAUC \t{}, PR-AUC \t{}, FAR \t{}\tGAP\t{}".format(
                        key, auc, pr_auc, normal_far, gap
                    )
                )
            else:
                logger.info(
            ...
            if logger == None:
                print(
                    "{}: \tAUC \t{:.4f}, PR-AUC \t{:.4f}, FAR \t{}\tGAP\t{:.4f}".format(
                        key, auc, pr_auc, false_alarm, gap
                    )
                )
            else:
                logger.info(
