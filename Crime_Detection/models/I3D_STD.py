...
class Unit3Dpy(torch.nn.Module):
    def __init__(
        ...
        if padding == "SAME":
            ...
            if stride[0] > 1:
                padding_shapes = [
                    get_padding_shape(kernel_size, stride, mod)
                    for mod in range(stride[0])
                ]
            else:
    ...
    def freeze_batch_norm(self):
        if self.freeze_bn:
            for name, module in self.backbone.named_modules():
                if isinstance(module, nn.BatchNorm3d) or isinstance(
