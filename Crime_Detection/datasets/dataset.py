...
class Test_Dataset_I3D(Dataset):
    ...
    def test_dict_annotation(self):
        ...
        for key in self.test_dict.keys():
            ...
            annotation = np.zeros(
                frames_num - frames_num % (self.segment_len), dtype=int
            )
            if len(anno) >= 2:
                front = anno[0]
                back = anno[1]
        ...
        for key in key_dict.keys():
            try:
                assert (
                    self.annotation_dict[key].shape[0] // self.segment_len
                    == key_dict[key]
                )
    ...
    def test_dict_annotation(self):
        ...
        for line in open(self.test_txt, "r").readlines():
            ...
            if anno_type == "1":
                ...
                label = "Normal"
                anno = np.zeros(
                    frames_num - frames_num % self.segment_len, dtype=np.uint8
                )
...
class Train_TemAug_Dataset_SHT_I3D(Dataset):
    def __init__(
        ...
        self.transforms = transforms.Compose(
            [
                ...
                transforms.MultiScaleCrop(
                    224, [1.0, 0.8], max_distort=1, fix_crop=True
                ),
    ...
    def get_rgb_diff(self, frames):
        ...
        diff = (
            diff - diff.min(dim=-1, keepdim=True)[0].min(dim=-2, keepdim=True)[0]
        ) / (
            diff.max(dim=-1, keepdim=True)[0].max(dim=-2, keepdim=True)[0]
            - diff.min(dim=-1, keepdim=True)[0].min(dim=-2, keepdim=True)[0]
        )
        diff = (
            diff.view([diff.shape[0], diff.shape[1] // 32, 32, diff.shape[2] // 32, 32])
            .mean(dim=-1)
            .mean(dim=-2)
        )
        diff = (
    ...
    def __getitem__(self, i):
        ...
        if not self.continuous_sampling:
            ...
            chosens = np.random.randint(0, vid_len - 1 - self.clip_num) + np.arange(
                0, self.clip_num
            )
        labels = []
        clips = []
        with h5py.File(self.h5_path, "r") as h5:
            for chosen in chosens:
                ...
                if chosen >= scores.shape[0]:
                    ...
                    score_1 = scores[
                        chosen * self.dataset_len // self.score_segment_len
                    ]
                    if (
                        chosen * self.dataset_len // self.segment_len + 1
                        < scores.shape[0]
                    ):
                        score_2 = scores[
                            chosen * self.dataset_len // self.score_segment_len + 1
                        ]
                ...
                if not self.hard_label:
                    if self.type != "Normal":
                        ...
                        label = np.array([1.0, 0.0]).astype(np.float32)
                    labels.append(label)
...
class Train_TemAug_Dataset_SHT_C3D(Dataset):
    def __init__(
        ...
        self.transforms = transforms.Compose(
            [
                ...
                # transforms.RandomCrop((112,112)),
                transforms.MultiScaleCrop(
                    112, [1.0, 0.8], max_distort=1, fix_crop=True
                ),
                transforms.RandomHorizontalFlip(),
                transforms.RandomGrayScale(),
                transforms.ClipToTensor(div_255=False),
    ...
    def __getitem__(self, i):
        ...
        if not self.continuous_sampling:
            ...
            chosens = np.random.randint(0, vid_len - self.clip_num - 1) + np.arange(
                0, self.clip_num
            )
        labels = []
        clips = []
