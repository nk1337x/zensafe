    ...
    for i in range(num_segments):
        if i < num_segments - 1:
            if int(random_p[i]) != int(random_p[i + 1]):
                random_p[i] = np.random.choice(
                    range(int(random_p[i]), int(random_p[i + 1]) + 1)
                )
            else:
...
def get_lr_scheduler(args, optimizer):
    lr_policy = lambda epoch: (
        (epoch + 0.5) / (args.warmup_epochs) if epoch < args.warmup_epochs else 1
    )
